...
- extract : this function is used to find text, and extract the IP address directly from the logs, the cowrie logs shows a new connection like this; **New connection: 139.199.80.137:51790 (x.x.x.x)** I needed to extract the IP out of the text
      - (\d{1,3}\.){3}\d{1,3} - 
      - \d{1,3} - 1-3 digits
      - {3} - this repeats the previous sequence 3 times
      - \d{1,3} - then the final group of digits
      - in total it covers the format of an IPv4 address, e.g. 192.148.12.34, 194.123.123.40
